...
class Policy(nn.Module):
	...
	def __init__(self, hidden_size,num_inputs,action_space):
		...
		self.lstm0 = nn.LSTM(1,self.hidden_lstm,2,batch_first=True)  #2 layers stacked LSTM
		self.lstm1 = nn.LSTM(1,self.hidden_lstm,2,batch_first=True)  #2 layers stacked LSTM
		self.lstm2 = nn.LSTM(1,self.hidden_lstm,2,batch_first=True)  #2 layers stacked LSTM
		self.lstm3 = nn.LSTM(1,self.hidden_lstm,2,batch_first=True)  #2 layers stacked LSTM
		self.lstm4 = nn.LSTM(1,self.hidden_lstm,2,batch_first=True)  #2 layers stacked LSTM
		self.lstm5 = nn.LSTM(1,self.hidden_lstm,2,batch_first=True)  #2 layers stacked LSTM
		self.lstm6 = nn.LSTM(1,self.hidden_lstm,2,batch_first=True)  #2 layers stacked LSTM
...
class NAF_LSTM:
	...
	def update_parameters(self,batch):
		...
		expected_state_action_values = reward_batch + (next_state_values * self.gamma* mask_batch)
