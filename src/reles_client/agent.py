...
class Online_Agent(threading.Thread):
	...
	def __init__(self,fd,cfg,memory,event,explore=True):
		...
		self.env = Env(fd = self.fd,time=self.cfg.getfloat('env','time'),k=self.cfg.getint('env','k'),
		alpha=self.cfg.getfloat('env','alpha'),b=self.cfg.getfloat('env','b'),c=self.cfg.getfloat('env','c'),
