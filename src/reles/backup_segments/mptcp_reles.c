	...
	if (meta_sk->sk_shutdown & RCV_SHUTDOWN &&
		...
		mptcp_for_each_sub(mpcb, mptcp) {
			...
			if (tcp_sk(sk)->mptcp->path_index == mpcb->dfin_path_index &&
			    mptcp_reles_is_available(sk, skb, zero_wnd_test, cwnd_limited))
				return sk;
		}
	}
	mptcp_for_each_sub(mpcb, mptcp) {
		...
		if (!mptcp_reles_is_available(sk, skb, zero_wnd_test, cwnd_limited)){
			continue;
		}
...
found:
	if (choose_sk) {
		...
		if (!mptcp_reles_is_available(choose_sk, skb, false, cwnd_limited))
			return NULL;
